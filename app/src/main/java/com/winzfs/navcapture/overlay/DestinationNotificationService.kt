...
class DestinationNotificationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_STOP -> {
                stopNotifications(clearSession = true)
    ...
    private fun stopNotifications(clearSession: Boolean) {
        if (clearSession) DisplaySessionStore.clear(this)
        getSystemService(NotificationManager::class.java).cancel(HEADS_UP_NOTIFICATION_ID)
    ...
    private fun createNotificationChannels() {
        ...
        manager.createNotificationChannel(
            NotificationChannel(
                ...
                description = "상태표시줄 아이콘과 알림창에 현재 목적지를 계속 표시합니다."
                setShowBadge(false)
            },
        )
        manager.createNotificationChannel(
